...
export function moveFocusByArrowKey(
  event,
  node,
  { orientation = "horizontal", selector = "a, button" } = {},
  ...
  const items = Array.from(node.querySelectorAll(selector)).filter(isVisibleFocusableItem)
...
export function useArrowKeyNav({ orientation = "horizontal", selector = "a, button" } = {}) {
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event) {
      moveFocusByArrowKey(event, node, { orientation, selector })
    }
